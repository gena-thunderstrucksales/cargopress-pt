...
if ( ! class_exists( 'PW_Number_Counter' ) ) {
	class PW_Number_Counter extends WP_Widget {

		private $current_widget_id;

		// Basic widget settings
		function widget_id_base() { return 'number-counter'; }
		function widget_name() { return __( 'Number Counter', 'cargopress-pt' ); }
		function widget_description() { return __( 'Number Counter widget used in Page Builder editor.', 'cargopress-pt' ); }
		function widget_class() { return 'widget-number-counter'; }

		// Return string of zeros
		function leading_zeros( $size ) {
			$s = '';
			while ( strlen( $s ) < $size ){
				$s = '0' . $s;
			}
		...
		public function widget( $args, $instance ) {
			...
			?>
				<div class="widget-number-counters" data-speed="<?php echo esc_attr( $instance['speed'] ); ?>">
				<?php foreach ( $counters as $counter ) : ?>
					<div class="number-counter">
						<i class="number-counter__icon  fa  <?php echo esc_attr( $counter['icon'] ); ?>"></i>
						<div class="number-counter__number  js-number" data-to="<?php echo esc_attr( $counter['number'] ) ?>"><?php echo esc_html( $this->leading_zeros( strlen( $counter['number'] ) ) ); ?></div>
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'speed' ) ); ?>"><?php _e( 'Counting Speed (in miliseconds):', 'cargopress-pt' ); ?></label>
			...
			<h4><?php _e( 'Counters:', 'cargopress-pt' ); ?></h4>

			<script type="text/template" id="js-pt-counter-<?php echo esc_attr( $this->current_widget_id ); ?>">
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'counters' ) ); ?>-{{id}}-title"><?php _e( 'Title:', 'cargopress-pt' ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'counters' ) ); ?>-{{id}}-number"><?php _e( 'Number:', 'cargopress-pt' ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'counters' ) ); ?>-{{id}}-icon"><?php _e( 'Icon:', 'cargopress-pt' ); ?></label>
					<small><?php printf( __( 'Click on the icon below or manually select from the %s website. Example of the input: fa-bed', 'cargopress-pt' ), '<a href="http://fontawesome.io/icons/" target="_blank">FontAwesome</a>' ); ?>.</small>
				...
				<p>
					...
					<a href="#" class="pt-remove-counter  js-pt-remove-counter"><span class="dashicons dashicons-dismiss"></span> <?php _e( 'Remove Counter', 'cargopress-pt' ); ?></a>
				</p>
			</script>
			<div class="pt-widget-about-us" id="counters-<?php echo esc_attr( $this->current_widget_id ); ?>">
				<div class="counters"></div>
				<p>
					<a href="#" class="button  js-pt-add-counter"><?php _e( 'Add New Counter', 'cargopress-pt' ); ?></a>
				</p>
			</div>
			<script type="text/javascript">
				(function() {
					...
					if ( _.isFunction( CargoPress.Utils.repopulateCounters ) ) {
						CargoPress.Utils.repopulateCounters( countersJSON, widgetId );
					}
				})();
			</script>
